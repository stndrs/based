...
pub fn sql_string_select_test() {
  ...
  let q =
    ...
    |> sql.select([sql.col("id"), sql.col("name")])
    |> sql.where(sql.eq(sql.col("id"), sql.int(1), of: sql.value))
    |> db.sql_string(database)

  let assert True = q == "SELECT id, name FROM users WHERE id = 1"
}

pub fn sql_string_select_no_params_test() {
  let database = db.driver() |> db.new(sql_adapter(), Conn)
  ...
  let assert True = q == "SELECT name FROM users"
}

pub fn sql_string_insert_test() {
  let database = db.driver() |> db.new(sql_adapter(), Conn)
  ...
  let assert True = q == "INSERT INTO users (name, age) VALUES ('Alice', 30)"
}

pub fn sql_string_update_test() {
  let database = db.driver() |> db.new(sql_adapter(), Conn)
  ...
  let assert True = q == "UPDATE users SET name = 'Bob' WHERE id = 1"
}

pub fn sql_string_delete_test() {
  let database = db.driver() |> db.new(sql_adapter(), Conn)
  ...
  let assert True = q == "DELETE FROM users WHERE id = 42"
}
